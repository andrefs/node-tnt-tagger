...
class Tagger {
  constructor(params={_unk:null, _T:false, _N:1000, _C:false}){
    this._uni = params._uni || new FreqDist();
    this._bi  = params._bi  || new ConditionalFreqDist();
    this._tri = params._tri || new ConditionalFreqDist();
    this._wd  = params._wd  || new ConditionalFreqDist();
    this._eos = params._eos || new ConditionalFreqDist();
    this._l1 = params._l1 || 0.0;
    this._l2 = params._l2 || 0.0;
    this._l3 = params._l3 || 0.0;
    this._N = params._N;
    this._C = params._C;
    this._T = params._T;

    this._unk = params._unk;

    // statistical tools (ignore or delete me)
    this.unknown = params.unknown || 0;
    this.known = params.known || 0;
  }

  async save(path){
    return fs.writeFile(path, JSON.stringify(this, null, 4));
  }

  static async loadFromFile(path){
    let obj = JSON.parse(await fs.readFile(path));

    obj._uni = FreqDist.load(obj._uni);
    obj._bi  = ConditionalFreqDist.load(obj._bi);
    obj._tri = ConditionalFreqDist.load(obj._tri);
    obj._wd  = ConditionalFreqDist.load(obj._wd);
    obj._eos = ConditionalFreqDist.load(obj._eos);
    return new Tagger(obj);
  }
  ...
  _compute_lambda(){
    ...
    for(const history of this._tri.conditions()){
      ...
      for (const tag of this._tri.get(history).keys()){
        ...
        if(c1 > c3 && c1 > c2){
          tl1 += this._tri.get(history).get(tag);
        }

        // if c2 is the maximum value
        else if(c2 > c3 && c2 > c1){
          tl2 += this._tri.get(history).get(tag);
        }

        // if c3 is the maximum value
        else if (c3 > c2 && c3 > c1){
          tl3 += this._tri.get(history).get(tag);
        }

        // if c3, and c2 are equal and larger than c1
        else if (c3 == c2 && c3 > c1){
          tl2 += this._tri.get(history).get(tag) / 2.0;
          tl3 += this._tri.get(history).get(tag) / 2.0;
        }

        // if c1, and c2 are equal and larger than c3
        // this might be a dumb thing to do....(not sure yet)
        else if (c2 == c1 && c1 > c3){
          tl1 += this._tri.get(history).get(tag) / 2.0;
          tl2 += this._tri.get(history).get(tag) / 2.0;
        }
  ...
  _tagword(sent, current_states){
    ...
    if(this._wd.conditions().includes(word)){
      this.known++;

      for(const [history, curr_sent_logprob] of current_states){
        let logprobs = [];

        for(const t of this._wd.get(word).keys()){


          const tC = [t, C];
          const p_uni = this._uni.freq(tC);
          const p_bi  = (this._bi.get(history.slice(-1)[0]) || new FreqDist()).freq(tC);
          const p_tri = (this._tri.get(history.slice(-2)) || new FreqDist()).freq(tC);
          const p_wd  = this._wd.get(word).get(t) / this._uni.get(tC);
